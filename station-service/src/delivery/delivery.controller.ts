...
export class DeliveryController {
  ...
  @ApiResponse({ status: 201, description: 'Livraison enregistrée avec succès' })
  @ApiResponse({ status: 400, description: 'Données invalides ou capacité dépassée' })
  @ApiResponse({ status: 404, description: 'Cuve ou fournisseur non trouvé' })
  @ApiResponse({ status: 409, description: 'Numéro de bon de livraison déjà existant' })
  async create(
  ...
  @ApiQuery({ name: 'supplierId', required: false, description: 'Filtrer par fournisseur' })
  @ApiQuery({ name: 'fuelTypeId', required: false, description: 'Filtrer par type de carburant' })
  @ApiResponse({ status: 200, description: 'Liste des livraisons de la station' })
  async findByStation(
  ...
  @ApiResponse({ status: 200, description: 'Liste des livraisons du fournisseur' })
  ...
  @ApiResponse({ status: 200, description: 'Livraison trouvée avec ses relations' })
