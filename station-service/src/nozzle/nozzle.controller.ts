...
export class NozzleController {
  ...
  @ApiResponse({ status: 400, description: 'Type de carburant incohérent avec la cuve' })
  @ApiResponse({ status: 403, description: 'Accès refusé' })
  @ApiResponse({ status: 404, description: 'Distributeur, cuve ou type de carburant non trouvé' })
  @ApiResponse({ status: 409, description: 'Référence déjà existante sur le distributeur' })
  ...
  @ApiResponse({ status: 200, description: 'Pistolet trouvé avec ses relations' })
  @ApiResponse({ status: 404, description: 'Pistolet non trouvé' })
  async findOne(@Param('id', ParseUUIDPipe) id: string) {
    return this.nozzleService.findOne(id);
  }
  ...
  @ApiResponse({ status: 400, description: 'Type de carburant incohérent avec la cuve' })
  @ApiResponse({ status: 403, description: 'Accès refusé' })
  @ApiResponse({ status: 404, description: 'Pistolet, cuve ou type de carburant non trouvé' })
  @ApiResponse({ status: 409, description: 'Référence déjà existante sur le distributeur' })
  async update(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: UpdateNozzleDto,
  ) {
    return this.nozzleService.update(id, dto);
  }
  ...
  async remove(@Param('id', ParseUUIDPipe) id: string) {
    await this.nozzleService.remove(id);
