...
export class ShiftController {
  ...
  @ApiResponse({
    status: 409,
    description: 'Un shift est déjà ouvert sur ce pistolet',
  })
  async startShift(
  ...
  @ApiResponse({
    status: 400,
    description: 'Shift non ouvert ou index invalide',
  })
  ...
  @ApiResponse({
    status: 400,
    description: 'Le shift doit être clôturé pour être validé',
  })
  ...
  @ApiQuery({
    name: 'pompisteId',
    required: false,
    description: 'Filtrer par pompiste',
  })
  @ApiQuery({
    name: 'status',
    required: false,
  ...
  @ApiQuery({ name: 'dateFrom', required: false, description: 'Date de début (ISO)' })
  @ApiQuery({ name: 'dateTo', required: false, description: 'Date de fin (ISO)' })
  @ApiResponse({ status: 200, description: 'Liste des shifts paginée' })
  async findAll(
    ...
    return this.shiftService.findAll(pagination, stationId, {
  ...
  @ApiResponse({
    status: 200,
    description: 'Historique des shifts du pistolet',
  })
  ...
  async findOne(
    ...
    return this.shiftService.findOne(id, stationId);
  }
}
